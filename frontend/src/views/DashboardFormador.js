...
const DashboardFormador = () => {
  ...
  try {
    const response = await axios.get(`https://backend-8pyn.onrender.com/api/utilizadores/utilizador/nome/${userId}`, {
      headers: {
...
//console.log("User ID:-------------************************", userId);
    try {
      setLoading(true);
      const response = await axios.get(`https://backend-8pyn.onrender.com/api/cursos/formador/${userId}`, {
