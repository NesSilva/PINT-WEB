...
const AvaliarAlunos = () => {
  ...
  const fetchCurso = async () => {
    try {
      const res = await axios.get(`https://backend-8pyn.onrender.com/api/cursos/${id_curso}`);
      setCurso(res.data);
    } catch (err) {
  ...
  const fetchInscritos = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`https://backend-8pyn.onrender.com/api/inscricoes/curso/${id_curso}`);
  ...
  const buscarDocumentosAluno = async (id_utilizador) => {
    try {
      const res = await axios.get(
        `https://backend-8pyn.onrender.com/api/documentos-avaliacao/utilizador/${id_utilizador}/curso/${id_curso}`
      );
      return res.data;
    } catch (err) {
  ...
  const enviarAvaliacao = async () => {
    ...
    try {
      await axios.post("https://backend-8pyn.onrender.com/api/progressos", {
