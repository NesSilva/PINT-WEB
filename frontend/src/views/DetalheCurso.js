...
const DetalhesCurso = () => {
    ...
    const fetchData = async () => {
      try {
        const [cursoRes, conteudosRes] = await Promise.all([
          axios.get(`https://backend-8pyn.onrender.com/api/cursos/${id_curso}`),
          axios.get(`https://backend-8pyn.onrender.com/api/conteudo/curso/${id_curso}`)
        ]);
        ...
        if (cursoRes.data.conteudo_upload) {
          try {
            const docsRes = await axios.get(
              `https://backend-8pyn.onrender.com/api/documentos-avaliacao/curso/${id_curso}`
            );
            
            const userId = localStorage.getItem('usuarioId');
            const userDocsRes = await axios.get(
              `https://backend-8pyn.onrender.com/api/documentos-avaliacao/utilizador/${userId}`
            );
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        'https://backend-8pyn.onrender.com/api/documentos-avaliacao/upload',
        formData,
        {
          headers: {
  ...
  const handleDeleteDocument = async (id) => {
    if (window.confirm('Tem certeza que deseja excluir este documento?')) {
      try {
        await axios.delete(`https://backend-8pyn.onrender.com/api/documentos-avaliacao/${id}`);
  ...
  const toggleUploadPermissao = async () => {
    try {
      ...
      await axios.put(`https://backend-8pyn.onrender.com/api/cursos/${id_curso}/toggle-upload`, {
