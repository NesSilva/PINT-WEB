    ...
    const carregarDados = async () => {
      try {
        const [cursosRes, categoriasRes, areasRes, formadoresRes] = await Promise.all([
          ...
          axios.get('https://backend-8pyn.onrender.com/api/utilizadores/formadores').catch(() => ({ data: { formadores: [] } }))
        ]);
  ...
  const handleReativarCurso = async () => {
    try {
        const response = await axios.put(
            `https://backend-8pyn.onrender.com/api/cursos/reativar/${cursoParaReativar.id_curso}`,
            {
